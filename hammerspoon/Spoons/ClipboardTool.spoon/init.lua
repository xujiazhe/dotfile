...
function obj:_showContextMenu(row)
  ...
  local menuTable = {
       ...
       { title = "Insert all snippets with linefeed", fn = hs.fnutils.partial(self.pasteAllWithDelimiter, self, row, "\n") },
       { title = "-" },
       ...
       { title = "全表倒转",   fn = hs.fnutils.partial(self.manageClip, self, row, 2) },
       { title = "-" },
       { title = "删除条目", disabled = true },
...
function obj:start()
   ...
   --self.timer = hs.timer.new(self.frequency, hs.fnutils.partial(self.checkAndStorePasteboard, self))
    ...
    pressedCommandC = false
    self.timer = hs.eventtap.new({hs.eventtap.event.types.keyDown, hs.eventtap.event.types.keyUp}, function (event)
        local ckey = event:getCharacters()
        if (ckey == 'c' or ckey == 'x') and event:getFlags().cmd then
            if event:getType() == hs.eventtap.event.types.keyDown then
                pressedCommandC = true
            else
                if pressedCommandC then
                    hs.timer.doAfter(0.12, hs.fnutils.partial(self.checkAndStorePasteboard, self))
                end
                pressedCommandC = false
            end
        end
...
function obj:pause(sec)
    if self.selectorobj ~= nil and not self.selectorobj:isVisible() then
        --self.timer:stop()
        self.timer:stop()
        --self.timer:setNextTrigger(sec)
        last_change = pasteboard.changeCount() + 2
        --current_clipboard = pasteboard.getContents()
        return true;
    else
        return false
